...
import { fromJS, List, Map, Item } from 'immutable';
import kelvinToCelsius from 'kelvin-to-celsius';

const selectWeatherDomain = state => state.get('weather');

const weatherIconMaker = (code, iconMap) => {
  const prefix = 'wi wi-';
  let icon = iconMap[code].icon;
...
export const selectWeather = () => createSelector(
  selectWeatherDomain, state => {
    const temp = kelvinToCelsius(parseInt(state.getIn(['weather', 'main', 'temp'])))
    ...
    const newWeather = state.getIn(['weather', 'weather'], List())
      .map((weather) => {
        const iconUrl = `http://openweathermap.org/img/w/${weather.get('icon')}.png`;
        const modernIcon = weatherIconMaker(code, iconMap)
